...
export default function WalletsPage() {
  ...
  const filteredWallets = useMemo(() => {
    ...
    return wallets.filter((wallet) => {
      ...
      const searchable = [wallet.owner_name, wallet.phone, wallet.role, wallet.wallet_id, wallet.id, wallet.kyc_status, currency].filter(Boolean).join(' ').toLowerCase();
    ...
    <AdminLayout title="Financial & Wallet Control Center" subtitle="Master Ledger for Supabase, Monime, and Flot synced balances.">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <section className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-200">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div><h3 className="font-semibold text-slate-900">Unified Customer Wallets</h3><p className="text-xs text-slate-500 mt-1">{filteredWallets.length} of {wallets.length} active wallets</p></div>
            </div>
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-2">
              <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><input value={walletSearch} onChange={(event) => setWalletSearch(event.target.value)} placeholder="Search customer, phone or ID..." className="w-full pl-9 pr-3 py-2 rounded-lg border border-slate-200 text-sm outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-400" /></div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredWallets.map((wallet) => {
                    ...
                    const isBusy = busyId === (wallet.wallet_id || wallet.id);
                    const safeId = String(wallet.wallet_id || wallet.id || 'N/A');
                      ...
                      <tr key={safeId} className="hover:bg-slate-50">
                        <td className="px-5 py-3 min-w-[220px]">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            <span className="text-[10px] text-slate-400 font-mono">{safeId.slice(0, 10)}…</span>
                          </div>
                        </td>
        ...
        <section className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-200">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div><h3 className="font-semibold text-slate-900">Wallet Transactions</h3><p className="text-xs text-slate-500 mt-1">Live Monime, Flot, and internal movements</p></div>
                  ...
                  {filteredTx.map((transaction) => {
                    ...
                    return (
                      <tr key={safeId} className="hover:bg-slate-50">
                        ...
                        <td className="px-5 py-3"><span className={`inline-flex items-center gap-1 text-xs font-medium ${transaction.direction === 'in' ? 'text-emerald-600' : 'text-slate-500'}`}>{transaction.direction === 'in' ? <ArrowDownLeft className="w-3.5 h-3.5" /> : <ArrowUpRight className="w-3.5 h-3.5" />}{transaction.direction === 'in' ? 'Deposit/Credit' : 'Payout/Debit'}</span></td>
