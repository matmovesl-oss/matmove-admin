...
export function PayoutsPage() {
  ...
  const fetchLivePayouts = async () => {
    ...
    try {
      ...
      if (data) {
        // Fetch Monime Payouts to update real statuses
        let monimeStatusMap: Record<string, string> = {};
        try {
          const res = await fetch('/api/get-payouts');
          const apiData = await res.json();
          if (apiData.payouts) {
            apiData.payouts.forEach((p:any) => {
               // Map Monime reference back to Supabase request ID
               if (p.metadata?.withdrawal_id) monimeStatusMap[p.metadata.withdrawal_id] = p.status;
            });
          }
        } catch(e) { console.error("Monime sync failed", e); }

        const mergedPayouts = data.map(p => ({
           ...p,
           status: monimeStatusMap[p.id] || p.status
        }));

        setPayouts(mergedPayouts);
        const pending = mergedPayouts.filter(p => p.status === 'pending' || p.status === 'processing');
        setStats({
          pendingCount: pending.length,
          pendingValue: pending.reduce((acc, curr) => acc + Number(curr.amount), 0),
          completedCount: mergedPayouts.filter(p => p.status === 'completed').length,
          failedCount: mergedPayouts.filter(p => p.status === 'failed' || p.status === 'rejected').length
        });
      }
